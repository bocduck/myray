...
func handle(c net.Conn) {
	...
	go func() {
		//Pack first few streams together
		//Not packing  HTTPUpgrade cause Shadowrocket not support
		var b bytes.Buffer
		b.Grow(4096)
		b.Write([]byte{0, 0}) //VLESS Response
		//b_debug := b.Len()
		buf := b.AvailableBuffer()
		n, err := dst.Read(buf[:cap(buf)])
		if n > 0 {
			b.Write(buf[:n])
			//fmt.Printf("b_debug: %d, %d, %d\n", b_debug, n, b.Len())
			_, errW := c.Write(b.Bytes())
			if errW != nil {
				errCh <- errW
				return
			}
